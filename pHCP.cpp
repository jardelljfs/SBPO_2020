...
int main(int argc, char *argv[]){
	...
	string instancia = "./instancias_u/";
	instancia += argv[1];

	cout<<instancia;	

	ifstream arq(instancia);
	if (!arq.is_open()){
		...
		IloExpr exp6(env);
			for(int i=0;i<N;i++)
				for(int j=0;j<N;j++){
					IloExpr mult(env);
					for(int l=0;l<N;l++)
						mult += y[i][j][k][l] + y[i][j][l][k];
					exp6 += w[i][j] * (mult- y[i][j][k][k]);
				}
	...
	env.out() << argv[1] << "\t" << cplex.getStatus() << "\t" << cplex.getBestObjValue() << "\t" << cplex.getObjValue() << "\t" << cplex.getNnodesLeft()<< "\t" << cplex.getTime();
	...
	ofstream arq("Results_Uncapacitated", ios::out | ios::app );

	arq << argv[1] << "\t" << cplex.getStatus() << "\t" << fixed << setprecision(2) << cplex.getBestObjValue() << "\t" << cplex.getNnodes() << "\t" << cplex.getNnodesLeft() << "\t" << cplex.getMIPRelativeGap() << "\t" <<cplex.getTime() << endl;
